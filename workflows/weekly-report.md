...
> **目标读者:Claude 本身**。把指定日期范围内用户在网页选稿 UI 中选定的条目合成一份 OneNote 兼容的 HTML 周报。
>
> 用户触发:"生成本周周报" / "weekly report" / "做这周的简报"
>
> **⚠️ 从 2026-05-12 起，选稿来源从 daily .md 文件的 `[x]` 勾选改为 `docs/data/selections.json`。生成周报前必须先执行 `git pull` 拉取最新选稿数据。**
...
开始之前:
1. **执行 `git -c http.proxy="" pull origin main`**，拉取最新的 `docs/data/selections.json`（用户在网页选稿 UI 中保存的选择存在 GitHub，必须先 pull 到本地）。
2. 检查 `docs/data/selections.json` 是否存在，以及目标日期范围内是否有选稿数据。
...
### 步骤 0:拉取最新选稿数据（必须最先执行，不可跳过）

```bash
git -c http.proxy="" pull origin main
```

- 用户在网页选稿 UI 中的勾选保存在 GitHub 上的 `docs/data/selections.json`，**本地文件可能落后于远端**。
- 不 pull 直接生成周报，会遗漏用户最近的选稿。
- pull 完成后，确认 `docs/data/selections.json` 存在且目标日期范围内有选中 ID，再继续步骤 1。
- 如果目标日期范围内无任何选中 ID，停止并提示用户："选稿区间内暂无选中条目，请先在网页选稿 UI 中完成选稿并保存后重试。"
...
**数据来源**：`docs/data/selections.json`（已在前置条件中 pull 到最新）

操作步骤：
1. 读取 `docs/data/selections.json`，格式为 `{ "YYYY-MM-DD": ["item-id-001", ...], ... }`
2. 提取本次周报日期范围内（上周二 ~ 本周一，或用户指定的日期范围）所有被选中的 item ID
3. 在 `docs/data/daily.json` 中根据 ID 查找对应的完整条目数据（company、title、summary_html、rating、group、sub_group、source_name、source_url）
4. 记录每条的所有元数据

**⚠️ 严格规则：Weekly Report 只使用 selections.json 中的选中 ID**

- **必须**：收集所有在 selections.json 中被选中的条目，无论 rating 是 ⭐、⭐⭐ 还是 ⭐⭐⭐
- **禁止**：因为 rating = 1 就跳过用户选中的条目
- **禁止**：因为 rating = 3 就强制包含未被选中的条目
- **原则**：Weekly 的筛选标准是 selections.json 里的选中 ID；rating 仅作为展示信息保留
- **违反此规则将导致用户选定的重要新闻被遗漏**

把所有条目汇总到一个**内部列表**。

**生成前确认**:
- 向用户展示"本次从 selections.json 收集到 X 条选中新闻（日期范围：MMDD~MMDD）"
- 列出所有条目的标题（公司名 + 日期 + 简要事件 + rating）
- 等待用户确认后再继续步骤 3

### 步骤 3:选中条目整理与去重

- 以步骤 2 从 selections.json 收集到的条目作为唯一输入,不按 rating 额外增删
- 允许做**去重与合并**:同一事件在多天或多来源重复出现时,合并为一条
- 允许做**表达压缩**:不改事实前提下合并近义表述,提升周报可读性
- 不允许因为"看起来不重要"而删除用户已勾选条目
...
### 步骤 8.5:生成 JSON 副产物（用于网页发布）

在生成 OneNote HTML 的同时(复用步骤 2-7 的内部数据),生成一份结构化 JSON 供网页展示使用。

**数据来源**:与周报正文保持一致,只使用本周期内 `docs/data/selections.json` 中选中的条目 ID 对应的数据；不按 rating 自动扩展、不补入未选中条目。
...
**写入文件**:`data/reports/YYYY-Wxx.json`（同一周重跑时直接覆盖）
...
- 写入 `data/reports/YYYY-Wxx.html`（同一周重跑时直接覆盖）
...
```
网页发布:本周周报(YYYY-Wxx)已写入 docs/data/weekly.json。
...
### 选中条目太少(<5 条)
- 不要硬凑。直接生成简短版周报
- 提醒用户:"本周期选中 X 条，可能是 daily 漏跑或选稿未及时同步。建议在网页选稿 UI 中补选后重新 pull 再生成。"

### 选中条目太多(>15 条)
- 不删用户已选中条目,优先通过同类事件合并与表述压缩降低篇幅
- 同一公司多条同类事件合并为一个主条目,子条目展开
- 必要时精简重复背景句,但保留所有被选中事件事实

### 同一事件在多份 daily 里都被选(重复)
- 去重,保留信息更完整的那条
- 合并多条的候选图 URL 和源链接
