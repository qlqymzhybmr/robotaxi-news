...
- 使用正则表达式 `^- \[x\]` 严格匹配勾选标记(Long 划过重点的)
- 完全忽略 `- [ ]` 的条目(未被选中)
- 记录每条的:公司名、日期、事件摘要、详细内容、候选图 URL、源链接、评级

**⚠️ 严格规则：Weekly Report 只看勾选状态，收集所有星级**

- **必须**：收集所有 `[x]` 勾选的条目，无论是 ⭐、⭐⭐ 还是 ⭐⭐⭐
- **禁止**：因为某条新闻是 ⭐（一星）就自动跳过它（只要用户勾选了 `[x]`）
- **禁止**：因为某条新闻是 ⭐⭐⭐（三星）但未勾选 `[ ]` 就强制包含它
- **原则**：Weekly 的筛选标准是用户的 `[x]` 勾选；rating 仅作为展示信息保留
- **违反此规则将导致用户标记的重要新闻被遗漏**

把所有条目汇总到一个**内部列表**。

**生成前确认**:
- 向用户展示"本次收集到 X 条勾选新闻"
- 列出所有条目的标题(公司名 + 日期 + 简要事件 + 评级)
- 等待用户确认后再继续步骤 3

### 步骤 3:勾选条目整理与去重

- 以步骤 2 收集到的 `[x]` 条目作为唯一输入,不按 rating 额外增删
...
**数据来源**:与周报正文保持一致,只使用本周期内用户手动勾选的 **[x]** 条目；不按 rating 自动扩展、不补入未勾选条目。
...
### 勾选条目太多(>15 条)
- 不删用户已勾选条目,优先通过同类事件合并与表述压缩降低篇幅
- 同一公司多条同类事件合并为一个主条目,子条目展开
- 必要时精简重复背景句,但保留所有被勾选事件事实
